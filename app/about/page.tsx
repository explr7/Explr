...
export default function AboutPage() {
  return (
    <div className="min-h-screen">
      {/* Header */}
      <section className="page-hero border-b border-[rgba(46,58,47,0.08)]" aria-labelledby="about-heading">
        <Container className="max-w-5xl" style={{display: 'flex', flexDirection: 'column', gap:"16px"}}>
          <Eyebrow>About explr</Eyebrow>
          <h1 id="about-heading" className="max-w-xl mb-6 text-4xl sm:text-5xl lg:text-6xl font-black text-[#1E2620] tracking-tight leading-[1.08]">
      ...
      <section className="py-20 sm:py-28" aria-labelledby="values-heading">
        <Container className="max-w-5xl" style={{display: 'flex', flexDirection: 'column', gap:"16px"}}>
          <div>
            ...
            <h2 id="values-heading" className="text-xl sm:text-4xl font-black text-[#1E2620] tracking-tight">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
            {beliefs.map((b) => (
              <div
                key={b.title}
                style={{padding: "24px"}}
                className="bg-white/80 backdrop-blur-xl border border-[rgba(46,58,47,0.1)] rounded-xl p-8 sm:p-10 shadow-xs hover:shadow-md hover:border-[#6B7F5B]/30 hover:-translate-y-1 transition-all duration-300 flex flex-col"
              >
                <span className="inline-flex items-center justify-center w-10 h-10 rounded-2xl bg-[#C96F4F]/10 text-[#C96F4F] font-mono text-xs font-bold tracking-widest mb-6 shrink-0">
                  {b.num}
                </span>
